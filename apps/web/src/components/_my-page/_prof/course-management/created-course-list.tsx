...
export default function CreatedCourseList({
	coursesWithCoverImage,
}: {
	coursesWithCoverImage: (FetchCreatedCoursesReturnType[number] & {
		coverImage?: string;
	})[];
}) {
	...
	// 検索フィルタリング
	const filteredCourses = coursesWithCoverImage.filter((course) =>
		...
		<div className="container relative z-10 mx-auto max-w-screen-xl px-4 py-6">
			<div className="mb-6 items-center justify-between space-y-3 sm:flex">
				...
				<div className="flex items-center gap-4">
					...
					/>
					<DefaultButton size="sm" onPress={() => setIsModalOpen(true)}>
						<SquarePlus />
						<p className="max-sm:hidden">講義の作成</p>
					</DefaultButton>
				</div>
			</div>
			...
			<CreateCourseForm isOpen={isModalOpen} onOpenChange={setIsModalOpen} />
		</div>
	);
}
